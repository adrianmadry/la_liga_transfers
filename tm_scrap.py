...
headings.append('Club name') 
    ...
    # Add column inside the list
index = headings.index('Left')
headings = headings[:index+1] + ['Left Club Country'] + headings[index+1:]

    # Write columns to CSV
with open('./data/Laliga_transfers.csv', 'w', newline='', encoding='UTF-8') as f:
                    writer = csv.writer(f)
    ...
    if first_col is not None:   # Exclude tables without headers
        if first_col.text.strip() == "In":  # Filtering only by IN transfers tables
            ...
            for row in rows:
                ...
                for cell in cells:
                    if cell.find('img'):    # Get 'Left Club Country' from <img>
                        img = cell.find('img')
                ...
                # Write data to CSV
                with open('./data/Laliga_transfers.csv', 'a+', newline='', encoding='UTF-8') as f:
                    writer = csv.writer(f)
